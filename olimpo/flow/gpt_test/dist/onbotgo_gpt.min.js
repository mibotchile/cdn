...
var F = /^async +/, X = /`(?:\\[\s\S]|\${(?:[^{}]|{(?:[^{}]|{[^}]*})*})*}|(?!\${)[^\\`])*`/g, J = /'(?:\\[\s\w"'\\`]|[^\n\r'\\])*?'/g, z = /"(?:\\[\s\w"'\\`]|[^\n\r"\\])*?"/g, wt = /[.*+\-?^${}()|[\]\\]/g;
function K(r) {
  return wt.test(r) ? r.replace(wt, "\\$&") : r;
}
function ie(r, t) {
  t.rmWhitespace && (r = r.replace(/[\r\n]+/g, `
`).replace(/^\s+|\s+$/gm, "")), X.lastIndex = 0, J.lastIndex = 0, z.lastIndex = 0;
  var e = t.prefixes, n = [
    e.h,
    e.b,
    e.i,
  ...
  function d(p, u) {
    ...
    for (var C; (C = o.exec(r)) !== null; ) {
      var S = C[1], v = C[2], x = C[3], M = C[4], E = C[5], T = C.index;
      if (S)
        ...
        if (v === "/*") {
          ...
          J.lastIndex = C.index;
          var I = J.exec(r);
          I ? o.lastIndex = J.lastIndex : D("unclosed string", r, C.index);
        } else if (v === '"') {
          z.lastIndex = C.index;
          var V = z.exec(r);
          V ? o.lastIndex = z.lastIndex : D("unclosed string", r, C.index);
        } else if (v === "`") {
          X.lastIndex = C.index;
          var L = X.exec(r);
          L ? o.lastIndex = X.lastIndex : D("unclosed string", r, C.index);
        }
      } else if (x)
        return y(T), i = T + C[0].length, s.lastIndex = i, a = E, M && u === "h" && (u = "s"), g.t = u, g;
    }
  ...
  function c(p, u) {
    ...
    for (var y; (y = s.exec(r)) !== null; ) {
      var C = y[1], S = y[2], v = y[3] || "", x;
      for (var M in e)
        if (e[M] === v) {
          x = M;
          break;
        }
      A(C, S), i = y.index + y[0].length, x || D("unrecognized tag type: " + v, r, i);
      var E = d(y.index, x), T = E.t;
      if (T === "h") {
        var H = E.n || "";
...
{{ /if}} {{/if}}\r
`;
var xe = globalThis && globalThis.__awaiter || function(r, t, e, n) {
  function o(s) {
...
class $ extends mt {
  ...
  renderBarWaveform(t, e, n, o) {
    ...
    for (let S = 0; S <= a; S++) {
      ...
      if (v > A) {
        const E = Math.round(y * l * o), T = Math.round(C * l * o), H = E + T || 1;
        let I = l - E;
        e.barAlign === "top" ? I = 0 : e.barAlign === "bottom" && (I = c - H), n[f](A * (b + p), I, b, H, u), A = v, y = 0, C = 0;
      }
      const x = Math.abs(s[S] || 0), M = Math.abs(i[S] || 0);
      x > y && (y = x), M > C && (C = M);
    }
    n.fill(), n.closePath();
  }
  ...
  renderChannel(t, e, n) {
    ...
    if (e.barWidth || e.barGap) {
      const v = e.barWidth || 0.5, x = e.barGap || v / 2, M = v + x;
      b % M !== 0 && (b = Math.floor(b / M) * M);
    }
    const p = Math.floor(Math.abs(a) * h), u = Math.floor(p + b * h), g = u - p, f = (v, x) => {
      this.renderSingleCanvas(t, e, n, s, Math.max(0, v), Math.min(x, l), o, i);
    }, A = this.createDelay(), y = this.createDelay(), C = (v, x) => {
      f(v, x), v > 0 && A(() => {
        C(v - g, x - g);
      });
    }, S = (v, x) => {
...
class tt extends Me {
  ...
  constructor(t) {
    ...
    }), this.plugins = [], this.decodedData = null, this.subscriptions = [], this.options = Object.assign({}, ke, t), this.timer = new Le();
...
</svg>`, Bt = '<svg  style="enable-background:new 0 0 32 32;" version="1.1" viewBox="0 0 32 32" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><path d="M28,14H18V4c0-1.104-0.896-2-2-2s-2,0.896-2,2v10H4c-1.104,0-2,0.896-2,2s0.896,2,2,2h10v10c0,1.104,0.896,2,2,2  s2-0.896,2-2V18h10c1.104,0,2-0.896,2-2S29.104,14,28,14z"/></svg>';
class ht {
}
...
var Je = Yt.exports;
const ze = /* @__PURE__ */ $t(Je), q = "onbotgo-chatcontainer";
class Ct extends B {
  constructor() {
    super();
