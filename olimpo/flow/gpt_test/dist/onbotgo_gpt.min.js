...
`).replace(/^\s+|\s+$/gm, "")), z.lastIndex = 0, J.lastIndex = 0, W.lastIndex = 0;
  ...
  function d(p, u) {
    var f = { f: [] }, b = 0, A = "c";
    u === "h" || u === "b" || u === "c" ? A = "n" : u === "r" && (f.raw = !0, u = "i");
    function y(X) {
      var Rt = r.slice(i, X), k = Rt.trim();
      if (A === "f")
        k === "safe" ? f.raw = !0 : t.async && O.test(k) ? (k = k.replace(O, ""), f.f.push([k, "", !0])) : f.f.push([k, ""]);
      else if (A === "fp")
        f.f[f.f.length - 1][1] += k;
      else if (A === "err") {
        if (k) {
          var Zt = Rt.search(/\S/);
    ...
    for (var C; (C = o.exec(r)) !== null; ) {
      var P = C[1], v = C[2], w = C[3], L = C[4], E = C[5], T = C.index;
      if (P)
        P === "(" ? (b === 0 && (A === "n" ? (y(T), A = "p") : A === "f" && (y(T), A = "fp")), b++) : P === ")" ? (b--, b === 0 && A !== "c" && (y(T), A = "err")) : b === 0 && P === "|" ? (y(T), A = "f") : P === "=>" && (y(T), i += 1, A = "res");
      else if (v) {
        if (v === "/*") {
          var I = r.indexOf("*/", o.lastIndex);
          I === -1 && F("unclosed comment", r, C.index), o.lastIndex = I + 2;
        } else if (v === "'") {
          J.lastIndex = C.index;
          var D = J.exec(r);
          D ? o.lastIndex = J.lastIndex : F("unclosed string", r, C.index);
        } else if (v === '"') {
          W.lastIndex = C.index;
          var Q = W.exec(r);
          Q ? o.lastIndex = W.lastIndex : F("unclosed string", r, C.index);
        } else if (v === "`") {
          z.lastIndex = C.index;
          var B = z.exec(r);
          B ? o.lastIndex = z.lastIndex : F("unclosed string", r, C.index);
        }
      } else if (w)
        return y(T), i = T + C[0].length, s.lastIndex = i, l = E, L && u === "h" && (u = "s"), f.t = u, f;
    }
  ...
  function c(p, u) {
    ...
    for (var y; (y = s.exec(r)) !== null; ) {
      var C = y[1], P = y[2], v = y[3] || "", w;
      for (var L in e)
        if (e[L] === v) {
          w = L;
          break;
        }
      A(C, P), i = y.index + y[0].length, w || F("unrecognized tag type: " + v, r, i);
      var E = d(y.index, w), T = E.t;
      if (T === "h") {
        var I = E.n || "";
        t.async && O.test(I) && (E.a = !0, E.n = I.replace(O, "")), E = c(E), b.push(E);
      } else if (T === "c") {
        if (p.n === E.n)
          return f ? (f.d = b, p.b.push(f)) : p.d = b, p;
        F("Helper start and end don't match", r, y.index + y[0].length);
      } else if (T === "b") {
        f ? (f.d = b, p.b.push(f)) : p.d = b;
        var D = E.n || "";
        t.async && O.test(D) && (E.a = !0, E.n = D.replace(O, "")), f = E, b = [];
      } else if (T === "s") {
        var Q = E.n || "";
        t.async && O.test(Q) && (E.a = !0, E.n = Q.replace(O, "")), b.push(E);
      } else
        b.push(E);
    }
    if (u)
      A(r.slice(i, r.length), !1), p.d = b;
    else
...
function U(r, t) {
  ...
  for (e; e < n; e++) {
    ...
    if (typeof s == "string") {
      ...
      var l = s.t, d = s.c || "", c = s.f, a = s.n || "", h = s.p || "", m = s.res || "", p = s.b, u = !!s.a;
      if (l === "i") {
        ...
        if (t.storage.nativeHelpers.get(a))
          ...
          var b = (u ? "await " : "") + "c.l('H','" + a + "')(" + Ot(t, m, s.d, h, u);
          p ? b += "," + ce(p, t) : b += ",[]", b += ",c)", o += "tR+=" + rt(b, c) + ";";
        }
      else
        l === "s" ? o += "tR+=" + rt((u ? "await " : "") + "c.l('H','" + a + "')({params:[" + h + "]},[],c)", c) + ";" : l === "e" && (o += d + `
`);
    }
  }
  return o;
...
const H = new we(), Z = "onbotgo-btn-whatsapp";
class dt extends M {
  constructor() {
    super(), this.innerHTML = this.renderHTML(this.html(), {}), this.onclick = () => {
      window.open(
        `https://wa.me/${H.number}?text=${H.msg}`
      );
    };
  }
  html() {
  ...
  return Mt(this, void 0, void 0, function* () {
    const s = yield fetch(r, e);
    {
      ...
      const c = (a, h) => Mt(this, void 0, void 0, function* () {
        ...
        const m = Math.round(d / l * 100);
        return t(m), i == null ? void 0 : i.read().then(({ done: p, value: u }) => c(p, u));
      });
...
function Fe(r, t, e, n, o = 5) {
  ...
  const i = (l) => {
    ...
    const h = (u) => {
      ...
      const f = u.clientX, b = u.clientY;
      if (a || Math.abs(f - d) >= o || Math.abs(b - c) >= o) {
        const { left: A, top: y } = r.getBoundingClientRect();
        a || (a = !0, e == null || e(d - A, c - y)), t(f - d, b - c, f - A, b - y), d = f, c = b;
      }
    }, m = (u) => {
    ...
    document.addEventListener("pointermove", h), document.addEventListener("pointerup", p), document.addEventListener("pointerleave", p), document.addEventListener("click", m, !0), s = () => {
...
class et extends At {
  ...
  renderBarWaveform(t, e, n, o) {
    const s = t[0], i = t[1] || t[0], l = s.length, { width: d, height: c } = n.canvas, a = c / 2, h = window.devicePixelRatio || 1, m = e.barWidth ? e.barWidth * h : 1, p = e.barGap ? e.barGap * h : e.barWidth ? m / 2 : 0, u = e.barRadius || 0, f = d / (m + p) / l, b = u && "roundRect" in n ? "roundRect" : "rect";
    n.beginPath();
    let A = 0, y = 0, C = 0;
    for (let P = 0; P <= l; P++) {
      const v = Math.round(P * f);
      if (v > A) {
        const E = Math.round(y * a * o), T = Math.round(C * a * o), I = E + T || 1;
        let D = a - E;
        e.barAlign === "top" ? D = 0 : e.barAlign === "bottom" && (D = c - I), n[b](A * (m + p), D, m, I, u), A = v, y = 0, C = 0;
      }
    ...
    const s = (i) => {
      ...
      for (let u = 0; u <= d; u++) {
        ...
        if (f > m) {
          ...
          n.lineTo(m, y), m = f, p = 0;
        }
        const b = Math.abs(l[u] || 0);
        b > p && (p = b);
      }
      n.lineTo(m, a);
    };
  ...
  renderSingleCanvas(t, e, n, o, s, i, l, d) {
    ...
    this.renderWaveform(t.map((f) => f.slice(s, i)), e, m);
    const p = a.cloneNode();
    d.appendChild(p);
  ...
  renderChannel(t, e, n) {
    ...
    let m = Math.min(et.MAX_CANVAS_WIDTH, c);
    if (e.barWidth || e.barGap) {
      const v = e.barWidth || 0.5, w = e.barGap || v / 2, L = v + w;
      m % L !== 0 && (m = Math.floor(m / L) * L);
    }
    const p = Math.floor(Math.abs(l) * h), u = Math.floor(p + m * h), f = u - p, b = (v, w) => {
...
const Ze = async (r) => await fetch(`${x.storage_url}/storage/upload`, {
  headers: {
    "Content-Type": "application/json"
  },
  method: "POST",
    ...
    e.defaults = {
      ...
      buildToast: function() {
        ...
        if (typeof this.options.destination < "u" && i.addEventListener(
          ...
          var m = o("x", this.options), p = o("y", this.options), u = this.options.position == "left" ? m : "-" + m, f = this.options.gravity == "toastify-top" ? p : "-" + p;
      ...
      for (var i = {
        ...
        var m = c[h].offsetHeight;
        a = a.substr(9, a.length - 1);
        var p = 15, u = window.innerWidth > 0 ? window.innerWidth : screen.width;
        u <= 360 ? (c[h].style[a] = d[a] + "px", d[a] += m + p) : s(c[h], "toastify-left") === !0 ? (c[h].style[a] = i[a] + "px", i[a] += m + p) : (c[h].style[a] = l[a] + "px", l[a] += m + p);
      }
      return this;
    };
    function o(i, l) {
...
class ft extends M {
  constructor() {
    ...
    e.onclick = () => n.toggle(), this.appendChild(e), this.appendChild(n), this.setStyles(this.componentStyles), H.active && H.position === "br" && this.setStyles({ bottom: "80px" });
  }
}
ft.tag = zt;
const en = () => ({
...
class yn {
  constructor({
    ...
    if (x.chatflowID = t, n && (x.projectPath = n), o && (x.botHost = o), s && (x.ssl = s), l && (x.welcomeMessage = l), x.chathubChannelId = e, !i)
      return;
    const { typography: c, colors: a, icon: h } = i;
    c && Object.keys(c).forEach(
      (m) => R.typography[m] = c[m]
    ), a && Object.keys(a).forEach(
      (m) => R.colors[m] = a[m]
    ), h && (R.customIcon = h), d.active && (H.active = !0, H.number = d.number, H.position = d.position.toLowerCase(), H.msg = d.msg);
  }
  init() {
    ...
    n.innerHTML = mn, t.prepend(n), document.body.appendChild(t), document.body.appendChild(e);
  }
