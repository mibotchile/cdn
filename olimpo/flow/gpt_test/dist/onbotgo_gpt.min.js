...
var Nt = (n, t, e) => t in n ? Vt(n, t, { enumerable: !0, configurable: !0, writable: !0, value: e }) : n[t] = e;
var A = (n, t, e) => (Nt(n, typeof t != "symbol" ? t + "" : t, e), e);
class Gt {
  constructor() {
    A(this, "typography", {
...
const Xt = {}, Yt = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: Xt
}, Symbol.toStringTag, { value: "Module" })), Tt = /* @__PURE__ */ Ut(Yt);
...
function j(n, t, e) {
  for (var r in t)
    Ft(t, r) && (t[r] != null && typeof t[r] == "object" && (r === "storage" || r === "prefixes") && !e ? n[r] = j(
      /*toObj[key] ||*/
      {},
      t[r]
    ) : n[r] = t[r]);
...
var W = /^async +/, V = /`(?:\\[\s\S]|\${(?:[^{}]|{(?:[^{}]|{[^}]*})*})*}|(?!\${)[^\\`])*`/g, N = /'(?:\\[\s\w"'\\`]|[^\n\r'\\])*?'/g, G = /"(?:\\[\s\w"'\\`]|[^\n\r"\\])*?"/g, St = /[.*+\-?^${}()|[\]\\]/g;
function U(n) {
  return St.test(n) ? n.replace(St, "\\$&") : n;
}
function Kt(n, t) {
  t.rmWhitespace && (n = n.replace(/[\r\n]+/g, `
`).replace(/^\s+|\s+$/gm, "")), V.lastIndex = 0, N.lastIndex = 0, G.lastIndex = 0;
  var e = t.prefixes, r = [
    e.h,
    e.b,
    e.i,
    ...
    return g && p ? g + "|" + U(p) : p ? U(p) : g;
  }, ""), o = new RegExp("([|()]|=>)|('|\"|`|\\/\\*)|\\s*((\\/)?(-|_)?" + // comments, strings
  U(t.tags[1]) + ")", "g"), i = new RegExp("([^]*?)" + U(t.tags[0]) + "(-|_)?\\s*(" + r + ")?\\s*", "g"), s = 0, l = !1;
  function h(g, p) {
    var u = { f: [] }, f = 0, b = "c";
    p === "h" || p === "b" || p === "c" ? b = "n" : p === "r" && (u.raw = !0, p = "i");
    function y($) {
      var wt = n.slice(s, $), T = wt.trim();
      if (b === "f")
        T === "safe" ? u.raw = !0 : t.async && W.test(T) ? (T = T.replace(W, ""), u.f.push([T, "", !0])) : u.f.push([T, ""]);
      else if (b === "fp")
        u.f[u.f.length - 1][1] += T;
      else if (b === "err") {
        if (T) {
          var $t = wt.search(/\S/);
          B("invalid syntax", n, s + $t);
        }
      } else
        u[b] = T;
      s = $ + 1;
    ...
    for (var C; (C = o.exec(n)) !== null; ) {
      var M = C[1], v = C[2], x = C[3], L = C[4], w = C[5], E = C.index;
      if (M)
        M === "(" ? (f === 0 && (b === "n" ? (y(E), b = "p") : b === "f" && (y(E), b = "fp")), f++) : M === ")" ? (f--, f === 0 && b !== "c" && (y(E), b = "err")) : f === 0 && M === "|" ? (y(E), b = "f") : M === "=>" && (y(E), s += 1, b = "res");
      else if (v) {
        if (v === "/*") {
          var H = n.indexOf("*/", o.lastIndex);
          H === -1 && B("unclosed comment", n, C.index), o.lastIndex = H + 2;
        } else if (v === "'") {
          N.lastIndex = C.index;
          var P = N.exec(n);
          P ? o.lastIndex = N.lastIndex : B("unclosed string", n, C.index);
        } else if (v === '"') {
          G.lastIndex = C.index;
          var _ = G.exec(n);
          _ ? o.lastIndex = G.lastIndex : B("unclosed string", n, C.index);
        } else if (v === "`") {
          V.lastIndex = C.index;
          var R = V.exec(n);
          R ? o.lastIndex = V.lastIndex : B("unclosed string", n, C.index);
        }
      } else if (x)
        return y(E), s = E + C[0].length, i.lastIndex = s, l = w, L && p === "h" && (p = "s"), u.t = p, u;
    }
    return B("unclosed tag", n, g), u;
  }
  function c(g, p) {
    g.b = [], g.d = [];
    var u = !1, f = [];
    ...
    for (var y; (y = i.exec(n)) !== null; ) {
      var C = y[1], M = y[2], v = y[3] || "", x;
      for (var L in e)
        if (e[L] === v) {
          x = L;
          break;
        }
      b(C, M), s = y.index + y[0].length, x || B("unrecognized tag type: " + v, n, s);
      var w = h(y.index, x), E = w.t;
      if (E === "h") {
        var H = w.n || "";
        t.async && W.test(H) && (w.a = !0, w.n = H.replace(W, "")), w = c(w), f.push(w);
      } else if (E === "c") {
        if (g.n === w.n)
          return u ? (u.d = f, g.b.push(u)) : g.d = f, g;
        B("Helper start and end don't match", n, y.index + y[0].length);
      } else if (E === "b") {
        u ? (u.d = f, g.b.push(u)) : g.d = f;
        var P = w.n || "";
        t.async && W.test(P) && (w.a = !0, w.n = P.replace(W, "")), u = w, f = [];
      } else if (E === "s") {
        var _ = w.n || "";
        t.async && W.test(_) && (w.a = !0, w.n = _.replace(W, "")), f.push(w);
      } else
        f.push(w);
    }
    if (p)
      b(n.slice(s, n.length), !1), g.d = f;
    else
      throw S('unclosed helper "' + g.n + '"');
    return g;
  }
  var a = c({ f: [] }, !0);
  if (t.plugins)
...
function Mt(n, t) {
  var e = Kt(n, t), r = "var tR='';" + (t.useWith ? "with(" + t.varName + "||{}){" : "") + O(e, t) + "if(cb){cb(null,tR)} return tR" + (t.useWith ? "}" : "");
  if (t.plugins)
...
function Ht(n, t, e, r, o, i) {
  var s = "{exec:" + (o ? "async " : "") + Pt(e, t, n) + ",params:[" + r + "]";
  return i && (s += ",name:'" + i + "'"), o && (s += ",async:true"), s += "}", s;
}
function te(n, t) {
  for (var e = "[", r = 0; r < n.length; r++) {
    var o = n[r];
    e += Ht(t, o.res || "", o.d, o.p || "", o.a, o.n), r < n.length && (e += ",");
  }
  return e += "]", e;
}
function Pt(n, t, e) {
  return "function(" + t + "){var tR='';" + O(n, e) + "return tR}";
}
function O(n, t) {
  ...
  for (e; e < r; e++) {
    ...
    if (typeof i == "string") {
      ...
      var l = i.t, h = i.c || "", c = i.f, a = i.n || "", d = i.p || "", m = i.res || "", g = i.b, p = !!i.a;
      if (l === "i") {
        ...
        if (t.storage.nativeHelpers.get(a))
          ...
          var f = (p ? "await " : "") + "c.l('H','" + a + "')(" + Ht(t, m, i.d, d, p);
          g ? f += "," + te(g, t) : f += ",[]", f += ",c)", o += "tR+=" + rt(f, c) + ";";
        }
      else
...
function Bt(n, t, e, r, o) {
  e(n[t], t).then(function(i) {
    r += i, t === n.length - 1 ? o(r) : Bt(n, t + 1, e, r, o);
  });
}
...
var oe = new J({}), gt = new J({
  ...
  try: function(n, t) {
    ...
    var e = "try{" + O(n.d, t) + "}", r = n.b[0];
    return e += "catch" + (r.res ? "(" + r.res + ")" : "") + "{" + O(r.d, t) + "}", e;
  },
  block: function(n, t) {
    q("block", n.b, n.f, !0);
...
function K(n, t) {
  var e = {};
  return j(e, at), t && j(e, t), n && j(e, n), e.l.bind(e), e;
}
function ut(n, t) {
...
gt.define("includeFile", de);
gt.define("extendsFile", pe);
...
class Q extends I {
  constructor() {
    super(), this.setStyles({
      fontFamily: k.typography.primary
    });
  }
}
Q.tag = ue;
const Y = "onbotgo-bubble";
class ft extends I {
  constructor(t) {
    super(), this.innerHTML = t, this.innerHTML = this.isIconValid(t) ? t : k.defaultIcon;
  }
  isIconValid(t) {
    var r, o, i, s;
    if (!t)
...
function Ce(n, t) {
  return typeof n[0] == "number" && (n = [n]), be(n), {
    duration: t,
    length: n[0].length,
...
var kt = globalThis && globalThis.__awaiter || function(n, t, e, r) {
  function o(i) {
    return i instanceof e ? i : new e(function(s) {
      s(i);
    });
...
function ve(n, t, e) {
  ...
  return kt(this, void 0, void 0, function* () {
    const i = yield fetch(n, e);
    {
      ...
      let h = 0;
      const c = (a, d) => kt(this, void 0, void 0, function* () {
        if (a)
          return;
        h += (d == null ? void 0 : d.length) || 0;
        const m = Math.round(h / l * 100);
        return t(m), s == null ? void 0 : s.read().then(({ done: g, value: p }) => c(g, p));
      });
...
function Se(n, t, e, r, o = 5) {
  ...
  const s = (l) => {
    ...
    const d = (p) => {
      ...
      const u = p.clientX, f = p.clientY;
      if (a || Math.abs(u - h) >= o || Math.abs(f - c) >= o) {
        const { left: b, top: y } = n.getBoundingClientRect();
        a || (a = !0, e == null || e(h - b, c - y)), t(u - h, f - c, u - b, f - y), h = u, c = f;
      }
    }, m = (p) => {
    ...
    document.addEventListener("pointermove", d), document.addEventListener("pointerup", g), document.addEventListener("pointerleave", g), document.addEventListener("click", m, !0), i = () => {
      document.removeEventListener("pointermove", d), document.removeEventListener("pointerup", g), document.removeEventListener("pointerleave", g), setTimeout(() => {
...
class tt extends At {
  ...
  renderBarWaveform(t, e, r, o) {
    const i = t[0], s = t[1] || t[0], l = i.length, { width: h, height: c } = r.canvas, a = c / 2, d = window.devicePixelRatio || 1, m = e.barWidth ? e.barWidth * d : 1, g = e.barGap ? e.barGap * d : e.barWidth ? m / 2 : 0, p = e.barRadius || 0, u = h / (m + g) / l, f = p && "roundRect" in r ? "roundRect" : "rect";
    r.beginPath();
    let b = 0, y = 0, C = 0;
    for (let M = 0; M <= l; M++) {
      const v = Math.round(M * u);
      if (v > b) {
        const w = Math.round(y * a * o), E = Math.round(C * a * o), H = w + E || 1;
        let P = a - w;
        e.barAlign === "top" ? P = 0 : e.barAlign === "bottom" && (P = c - H), r[f](b * (m + g), P, m, H, p), b = v, y = 0, C = 0;
      }
      const x = Math.abs(i[M] || 0), L = Math.abs(s[M] || 0);
      x > y && (y = x), L > C && (C = L);
    }
    r.fill(), r.closePath();
  }
  renderLineWaveform(t, e, r, o) {
    const i = (s) => {
      ...
      let m = 0, g = 0;
      for (let p = 0; p <= h; p++) {
        const u = Math.round(p * d);
        if (u > m) {
          const b = Math.round(g * a * o) || 1, y = a + b * (s === 0 ? -1 : 1);
          r.lineTo(m, y), m = u, g = 0;
        }
  ...
  renderSingleCanvas(t, e, r, o, i, s, l, h) {
    ...
    this.renderWaveform(t.map((u) => u.slice(i, s)), e, m);
    const g = a.cloneNode();
    h.appendChild(g);
  ...
  renderChannel(t, e, r) {
    ...
    let m = Math.min(tt.MAX_CANVAS_WIDTH, c);
    if (e.barWidth || e.barGap) {
      const v = e.barWidth || 0.5, x = e.barGap || v / 2, L = v + x;
      m % L !== 0 && (m = Math.floor(m / L) * L);
    }
    const g = Math.floor(Math.abs(l) * d), p = Math.floor(g + m * d), u = p - g, f = (v, x) => {
      this.renderSingleCanvas(t, e, r, i, Math.max(0, v), Math.min(x, a), o, s);
    }, b = this.createDelay(), y = this.createDelay(), C = (v, x) => {
      f(v, x), v > 0 && b(() => {
        C(v - u, x - u);
      });
    }, M = (v, x) => {
...
class et extends xe {
  ...
  constructor(t) {
    ...
    }), this.plugins = [], this.decodedData = null, this.subscriptions = [], this.options = Object.assign({}, Ee, t), this.timer = new Me();
  ...
  exportPeaks({ channels: t = 1, maxLength: e = 8e3, precision: r = 1e4 } = {}) {
    ...
    for (let s = 0; s < o; s++) {
      ...
      for (let d = 0; d < h; d++) {
        const m = Math.round(d * c), g = l[m];
        a.push(Math.round(g * r) / r);
      }
      i.push(a);
    }
    return i;
...
const Z = new Ie(), Re = async (n) => await fetch(`${Z.gpt_url}/chat`, {
  headers: {
    "Content-Type": "application/json"
  },
  method: "POST",
  body: JSON.stringify(n)
}).then((t) => t.json()), Te = `<input\r
...
class yt extends I {
  constructor() {
    ...
    A(this, "input");
    this.innerHTML = this.renderHTML(Te, { sendIcon: qt }), this.input = this.querySelector("input"), this.sendIcon = this.querySelector(Q.tag), this.input.style.fontFamily = k.typography.primary, this.setStyles(this.defaultStyles);
  }
  onSubmit(e) {
...
const He = `<onbotgo-box style="padding: 20px; max-width: 100%" id="onbotgo-chatheader">\r
  <img\r
    src=""\r
    style="margin-left: -20px; width: 40%"\r
    height="80"\r
...
</svg>`, It = '<svg  style="enable-background:new 0 0 32 32;" version="1.1" viewBox="0 0 32 32" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><path d="M28,14H18V4c0-1.104-0.896-2-2-2s-2,0.896-2,2v10H4c-1.104,0-2,0.896-2,2s0.896,2,2,2h10v10c0,1.104,0.896,2,2,2  s2-0.896,2-2V18h10c1.104,0,2-0.896,2-2S29.104,14,28,14z"/></svg>';
class F {
}
A(F, "messageHistoryId", {
...
const De = "data:image/jpeg;base64,", z = "onbotgo-chatcontainer";
class Ct extends I {
  constructor() {
    super();
  ...
  onSubmit(e) {
    ...
    structuredClone(this.messagesHistory).splice(0, 1), console.log(
      F.messageHistoryId,
      F.messageHistoryId.get()
    );
    const o = {
      message: e,
      unique_id: F.messageHistoryId.get() ?? ""
    };
    ...
    ]), e && this.addMessages([{ message: e, type: "userMessage" }]), F.messagesHistory.add([
      {
        message: e,
        type: "userMessage"
      }
    ]), this.renderMessages([{ type: "LoadingMessage" }]), this.updateScrollbar(), this.isRecordSelected = !1, this.attachedRecord = null, this.chatInput.getChild("input").value = "", this.getChild("#onbotgo-chatinput").style.display = "grid", this.getChild("#onbotgo-attachFileTemplateContainer").style.display = "grid", this.getChild("#onbotgo-micrecord").style.display = "none", this.updateScrollbar(), Re(o).then((i) => {
      var s, l;
      console.log(i), i != null && i.unique_id && F.messageHistoryId.set(i.unique_id), (l = (s = i == null ? void 0 : i.data) == null ? void 0 : s.process) != null && l.length && i.data.process.forEach((h) => {
        this.addMessages([
          {
            type: h.role,
            name: h.name,
  ...
  renderAttachTemplate() {
    this.getChild("#onbotgo-attachFileTemplateContainer").innerHTML = this.renderHTML(Pe, {
  ...
  render() {
    this.innerHTML = this.renderHTML(He, {
      logo: Be,
      micIcon: We,
      paperclipIcon: Lt,
      attachedFiles: this.attachedFiles,
...
const Oe = (n) => ({
  ...
  [`${z} .onbotgo-messageContainer`]: {
    display: "flex !important"
  },
  [`${z} .onbotgo-chat-footer`]: {
...
class pt extends I {
  constructor() {
    ...
    const e = new ft(k.customIcon), r = new Ct();
...
function qe({ element: n, styles: t }) {
  const e = t.reduce(
    ...
    {}
  ), r = document.createElement("style"), o = Qe(e);
...
class zt extends I {
  constructor() {
    ...
    let r = "black";
    t === "primary" && (r = k.colors.primary), t === "danger" && (r = "red"), e && (this.querySelector("svg").style.width = `calc(${e} - 16px)`, this.querySelector("svg").style.height = `calc(${e} - 16px)`, this.setStyles({ width: e, height: e })), this.setStyles({ color: r });
    const o = new Q();
    o.setStyles({
      backgroundColor: r,
      opacity: 0.2,
...
class Qt extends I {
  constructor() {
    ...
    const t = this.getAttribute("color"), e = this.getAttribute("size");
    let r = "black";
    t === "primary" && (r = k.colors.primary), e && this.setStyles({ width: e, height: e }), this.setStyles({
...
Qt.tag = Ve;
const Ne = `<onbotgo-iconbutton color="danger" size="36px" id="onbootgo-delete-record">\r
...
<!-- END STOP-->\r
`, Ge = `<svg viewBox="0 0 28 28" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:sketch="http://www.bohemiancoding.com/sketch/ns">\r
    <defs>\r
</defs>\r
...
const Ye = ["audio/webm", "audio/wav", "audio/mpeg", "audio/mp4", "audio/mp3"];
class vt extends Xe {
  constructor(t) {
    var e;
  ...
  startRecording() {
    return Rt(this, void 0, void 0, function* () {
      const t = this.stream || (yield this.startMic()), e = this.mediaRecorder || new MediaRecorder(t, { mimeType: this.options.mimeType || Ye.find((o) => MediaRecorder.isTypeSupported(o)), audioBitsPerSecond: this.options.audioBitsPerSecond });
...
class _t extends I {
  ...
  render() {
    this.innerHTML = this.renderHTML(Ne, {
...
class Ke {
  constructor({ chatflow: t, theme: e, welcomeMessage: r }) {
    if (Z.chatflowID = t, console.log(r), r && (Z.welcomeMessage = r), !e)
      return;
    const { typography: o, colors: i, icon: s } = e;
    o && Object.keys(o).forEach(
      (l) => k.typography[l] = o[l]
