...
`).replace(/^\s+|\s+$/gm, "")), Y.lastIndex = 0, Q.lastIndex = 0, V.lastIndex = 0;
  ...
  function h(p, g) {
    ...
    for (var v; (v = o.exec(n)) !== null; ) {
      var w = v[1], I = v[2], x = v[3], L = v[4], A = v[5], R = v.index;
      if (w)
        ...
        return y(R), s = R + v[0].length, i.lastIndex = s, l = A, L && g === "h" && (g = "s"), u.t = g, u;
    }
...
class $ extends mt {
  ...
  renderBarWaveform(t, e, r, o) {
    ...
    for (let w = 0; w <= l; w++) {
      ...
      const x = Math.abs(i[w] || 0), L = Math.abs(s[w] || 0);
      x > y && (y = x), L > v && (v = L);
    }
    r.fill(), r.closePath();
  }
  ...
  renderChannel(t, e, r) {
    ...
    if (e.barWidth || e.barGap) {
      const I = e.barWidth || 0.5, x = e.barGap || I / 2, L = I + x;
...
const k = new Me(), kt = async (n) => await fetch(`${k.gpt_url}/chat`, {
  headers: {
...
class vt extends M {
  ...
  initMessagesWebsocket(e) {
    const r = `ws://endpoint-prod-chatgpt.mibot.cl:8080/messages/${e}/ws`;
    this.messagesWebsocket = new WebSocket(r), console.log(socket), this.messagesWebsocket.onmessage = (o) => {
